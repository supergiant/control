...
sudo openssl genrsa -out /etc/kubernetes/pki/kubelet.key 2048
sudo openssl req -new -key /etc/kubernetes/pki/kubelet.key -out /etc/kubernetes/pki/kubelet.csr -subj "/CN=system:node:${HOSTNAME}/O=system:nodes"
...
$(sudo kubectl --kubeconfig=/root/.kube/config get csr ${HOSTNAME} -o jsonpath='{.status.certificate}' | base64 -d)
EOF"
