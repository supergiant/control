...
sud bash -c "cat > /etc/kubernetes/ssl/ca.pem <<EOF
{{ .CACert }}EOF"

sudo bash -c "cat > /etc/kubernetes/ssl/ca-key.pem <<EOF
{{ .CAKey }}EOF"

sudo bash -c "sed -e 's/{MASTER_HOST}/`curl ipinfo.io/ip`/' < /etc/kubernetes/ssl/openssl.cnf.template > /etc/kubernetes/ssl/openssl.cnf.public"
sudo bash -c "sed -e 's/{PRIVATE_HOST}/{{ .MasterHost }}/' < /etc/kubernetes/ssl/openssl.cnf.public > /etc/kubernetes/ssl/openssl.cnf"
...
sudo openssl x509 -req -in /etc/kubernetes/ssl/worker.csr -CA /etc/kubernetes/ssl/ca.pem -CAkey /etc/kubernetes/ssl/ca-key.pem -CAcreateserial -out /etc/kubernetes/ssl/worker.pem -days 365
...
sudo openssl x509 -req -in /etc/kubernetes/ssl/admin.csr -CA /etc/kubernetes/ssl/ca.pem -CAkey /etc/kubernetes/ssl/ca-key.pem -CAcreateserial -out /etc/kubernetes/ssl/admin.pem -days 365
